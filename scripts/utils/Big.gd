...
func sufix(enable_millions: bool = true) -> String:
	return _format_value(value, enable_millions)
...
static func _format_value(n: float, enable_millions: bool) -> String:
	...
	if absn < _thresholds[start_i]:
		...
		var decimals := MAX_DECIMALS_SMALL
		s = "%.*f" % [decimals, n]
		s = s.rstrip("0").rstrip(".")
